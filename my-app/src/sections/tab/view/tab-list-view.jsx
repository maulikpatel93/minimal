...
import { TabListApi,TabDeleteApi,TabDetailApi } from 'src/redux/slices/tabSlice';
...
export default function TabListView() {
  ...
  const tabList = useSelector((state) => state.tab.list);
  const loadingList = useSelector((state) => state.tab.loading.list);
  const totalRows = tabList ? tabList.total : 0;
  const currentPage = tabList ? tabList.current_page : 1;
  ...
  const handleEditRow = useCallback(
    (id) => {
      dispatch(TabDetailApi({ id: id })).then(action => {
        if(action.meta.requestStatus === "fulfilled"){
          // dispatch(ModuleDropdownListApi()).then(module=>{
          // })
          router.push(paths.dashboard.roleManagement.tab.edit(id));
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        return (
          <Typography noWrap variant="icon">
            {row?.submodule && row?.submodule?.title}
          </Typography>
        );
      },
    },
      ...
      >
        ...
        >
          ...
          <DataGrid
            ...
            rows={tabList && tabList?.data ? tabList.data : []}
            columns={columns}
            loading={loadingList}
      ...
      <ConfirmDialog
        ...
        action={
          <Button
            ...
            color="error"
            onClick={() => {
              dispatch(TabDeleteApi(selectedRowIds)).then((action) => {
                if (action.meta.requestStatus === 'fulfilled') {
                  dispatch(
                    TabListApi({
                      q: value,
